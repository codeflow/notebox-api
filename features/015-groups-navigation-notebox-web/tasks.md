...
- [x] **T-03 · The Navigator in the layout — render, one fetch, empty and error states** ✔ 2026-08-26, `npm run verify` green (434 tests, 427 → 434; satellite `a93e01e`). Single-fetch asserted by **counting MSW requests**. Beyond the file list: `app/(app)/layout.test.tsx` needed `useSearchParams` in its `next/navigation` mock now that the Navigator is chrome — extended (it now also asserts the Navigator renders) rather than merely patched. One of my own assertions was wrong and corrected: RabbitMQ appears **twice** (Infrastructure + Ungrouped), which is the multi-group case rendering correctly, so the test now pins exactly 2.
...
- [x] **T-05 · Groups management — two panels, CRUD dialogs, the aggregates as received** ✔ 2026-08-26, `npm run verify` green (454 tests, 442 → 454; satellite `a8b1e92`). All three "assert rather than assume" items done: the **absence** of a domain control (combobox, label and literals all checked), the delete dialog's **wording** asserted as text, and `averageStatus: null` → "No tasks" **mutation-checked** (`?? 0` fails 2 tests). 24 message keys line-parallel; the pt test also asserts the en string is absent.
