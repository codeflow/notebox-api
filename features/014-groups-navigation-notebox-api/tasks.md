...
- [x] **T-03 · Group endpoints + edge contract — DTOs, `@ValidGroupDomain`, 10 message keys, `GroupResource`** ✔ 2026-08-23, verify green (336 tests, 316 → 336; `57e6e86`). `OpenApiCoverageTest` extended with the `/api/groups` paths (NFR-06 — the file was not in the task list; the test is the project's NFR-06 enforcement point). **Deviation raised for the audit:** the scenario "A group cannot be nested" says the request *is rejected*; it is not — unknown JSON properties are ignored project-wide, so a stated `parentId` yields 201 and is dropped. OQ-04 holds in substance (no parent stored or returned) but not in the literal wording; making it reject would change deserialization for every endpoint.
...
- [x] **T-06 · Navigation tree — distinct projections, `NavigationService`, `GET /navigation`** ✔ 2026-08-23, verify green (367 tests, 359 → 367; `ed72de0`). `AnnotationTypeRepository.listAllInTenantByName` added (not in the file list — the tree needs the structural axis name-ordered) and `OpenApiCoverageTest` extended with `/api/navigation`. A null `groupId` in the DISTINCT projection **is** the Ungrouped signal, so no second query is needed.
