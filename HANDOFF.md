...
> **Updated:** 2026-08-26 (US-3.1 delivered)
...
- **Just landed (2026-08-26):** feat-015 — the Navigator, the Groups screen and group assignment (US-3.1 web half). Squash `4ef2a0f`, audit Round 1 pass w/ findings (F-01..F-03 closed same day), 464 tests, branch + PR CI green. **US-3.1 is delivered on both sides; E3 is done.**
...
## In flight
- **Nothing mid-feature.** `wf next` → the `delivery` container: either scope **E5 (i18n, US-5.1/US-5.2)** — the only untouched epic left — or close delivery toward `release`.
