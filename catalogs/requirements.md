...
| FR-20 | **A subtask records the moment it was marked done**, and that moment is erased when it is un-marked. Server-written and readable, never client-supplied — a client that states it is rejected, in the BR-06/BR-07 poison-field style. Subtasks completed before this existed carry no moment and none is invented for them. *(decisão humana 2026-09-02, OQ-38)* | Must | decisão humana 2026-09-02 + OQ-38, BR-06/BR-07 | US-4.1 | building |
...
| 2026-09-03 | v4 | Added **FR-20** (a subtask records when it was completed) from a direct human decision recorded as OQ-38. **Root cause it closes:** FR-11 enumerates a subtask's attributes as "name, dates, optional card, done flag" and says nothing about *when* completion happened, so the feature had no originating requirement — writing its spec against FR-11 would have stretched a requirement over surface it does not name. Deriving the fact is impossible: without a stored moment, "finished late" reads as *done and the end date is past*, which turns true for work delivered on time as soon as today passes that date. Delivered by feat-029 (api) + feat-030 (web). |
