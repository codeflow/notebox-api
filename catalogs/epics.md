...
| US-4.2 | As a tenant member, I want task dates derived from subtasks, a card link, and rich-text details, so a task is self-contained. | FR-12, FR-13, FR-14 · BR-07 | building | feat-012-task-details-notebox-api (api, issue #13) — **audit PASS w/ findings 2026-08-17** (Round 1; 268 tests; 0 blockers; 5 hardening items + 2 backlog notes, see `features/012-task-details-notebox-api/audit.md`), publish/PR next · feat-013-task-details-notebox-web (notebox-web, issue #12) — not started; **rollout note:** under PUT-replace semantics the shipped feat-011 UI clears API-set cards/details until feat-013 ships — the pair should reach `main` together |
