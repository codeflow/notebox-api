...
| ID | User Story | FR/BR | Status | Feature |
|----|------------|-------|--------|---------|
| US-4.1 | As a tenant member, I want tasks with subtasks whose completion drives the task's progress, so status reflects real work. | FR-10, FR-11, **FR-20** · BR-06 | delivered | feat-010-tasks-notebox-api (api, issue #11) — **merged into `develop` 2026-08-14** (PR #12, squash `fb71054`; 212 tests) · feat-011-tasks-notebox-web (notebox-web, issue #10) — **merged into `develop` 2026-08-15** (PR #11, squash `6324257`; audit Round 1 pass w/ findings closed by hardening; 355 tests + live pass). Both sides delivered **on develop**; promotion to `main` pending. **FR-20 extends this story (2026-09-03):** feat-029-subtask-completion-notebox-api (api) — implemented, audit pass-with-findings, not yet published · feat-030-subtask-completion-notebox-web (web) — not started. Promote the pair together. |
...
| 2026-09-03 | v23 | **FR-20 added to US-4.1** (a subtask records when it was completed), from a direct human decision recorded as OQ-38. The story stays `delivered` for FR-10/FR-11 while the FR-20 pair is in flight — feat-029 (api) is code-complete on `feature/subtask-completion` with 429 tests green and an audit verdict of pass-with-findings (0 blockers, 19 findings, the scenario-honesty axis not run by the product owner's decision); feat-030 (web) has not started. **Rollout hazard:** under PUT-replace an omitted `done` sets false and now ERASES the recorded moment, so a client that does not echo `done` silently destroys a completion — promote the pair together, as with feat-012/013 and feat-014/015. |
