...
| ID | User Story | FR/BR | Status | Feature |
|----|------------|-------|--------|---------|
| US-2.1 | As a tenant member, I want to create/edit/delete annotations of a type, so I record real data. | FR-04, FR-06, FR-18 · BR-03, BR-05, BR-10 | building | feat-005-annotation-records-notebox-api (api) — **merged into `develop` 2026-08-11** (PR #6, squash `0f056d7`; audit Round 4 pass, 133 tests green); promotion to `main` pending · feat-006-annotation-records-notebox-web (notebox-web) — **audit pass 2026-08-11 (Round 2)**, 13/13 tasks, verify green 277 tests; awaiting publish + review. **Promotion blocker:** API-side C-08 sanitization gap (OQ-19 dependency) still unscoped. Includes secret-value encryption + audited reveal (FR-18); enforces type-delete-block when records exist (OQ-14) |
